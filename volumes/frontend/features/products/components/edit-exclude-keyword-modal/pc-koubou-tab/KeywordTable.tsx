...
import { deletePcKoubouCrawlSettingExcludeKeyword } from '@/features/products/server-actions/pcKoubouCrawlSettingExcludeKeywordQuery'
...
const KeywordTable = ({
  ...
  setCondition: Dispatch<SetStateAction<PcKoubouCrawlSettingExcludeKeyword | undefined>>
}) => {
  ...
  const destroy = async (id: string, productId: string) => {
    const result = await deletePcKoubouCrawlSettingExcludeKeyword(id, productId, pathname)
  ...
  return (
    <>
      <table className='table'>
        ...
        <tbody>
          {data.product.pcKoubouCrawlSetting.pcKoubouCrawlSettingExcludeKeywords.map(
            (condition) => (
              <tr key={condition.id}>
