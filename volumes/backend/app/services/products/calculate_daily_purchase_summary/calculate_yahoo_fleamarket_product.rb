...
module Products
  module CalculateDailyPurchaseSummary
    class CalculateYahooFleamarketProduct
      def self.call(...)
        new(...).call
      end
      ...
      def calculate_result_map
        @calculate_result_map ||=
          ...
          .select(
            <<~SQL
              DATE(yahoo_fleamarket_products.bought_date) AS date,
              AVG(yahoo_fleamarket_products.price) AS average_purchase_price,
              COUNT(yahoo_fleamarket_products.id) AS purchase_count
            SQL
          )
      ...
      def save(calculate_results)
        YahooFleamarketDailyPurchaseSummary.transaction do
          YahooFleamarketDailyPurchaseSummary.upsert_all(calculate_results)
        end
      end
    end
  end
