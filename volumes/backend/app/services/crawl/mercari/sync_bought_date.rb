module Crawl
  module Mercari
    class SyncBoughtDate
      ...
      def call
        # NOTE: 直列処理の都度コミットで良いのでトランザクションブロックは張らない
        product.mercari_products.unpublished.where(bought_date: nil).each do |mercari_product|
          bought_date = fetch_bought_date(mercari_product)
          ...
          Rails.logger.error(
            "SyncBoughtDate is failed. { mercari_id: #{mercari_product.mercari_id}, exception: #{e.message} }"
          )
        end
      end
      ...
      def fetch_bought_date(mercari_product)
